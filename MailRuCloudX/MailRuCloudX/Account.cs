...
namespace MailRuCloudApi
{
    ...
    public class Account
    {
        ...
        public async Task<bool> LoginAsync()
        {
            ...
            string reqString = string.Format("Login={0}&Domain={1}&Password={2}", 
                this.LoginName, ConstSettings.Domain, HttpUtility.UrlEncode(this.Password));

            byte[] requestData = Encoding.UTF8.GetBytes(reqString);

            var request = (HttpWebRequest)WebRequest.Create(string.Format(
                "{0}/cgi-bin/auth", ConstSettings.AuthDomen));
