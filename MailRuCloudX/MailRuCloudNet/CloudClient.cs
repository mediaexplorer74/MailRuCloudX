...
namespace MailRuCloudClient
{
    ...
    public class CloudClient
    {
        ...
        public /*async Task<string>*/ string GetFileOneTimeDirectLink(string publicLink)
        {
            Debug.Assert(
                !string.IsNullOrEmpty(publicLink) && publicLink.StartsWith(Urls.PublicLink),
                "Is not correct public link.");

            /*await*/ this.Account.CheckAuthorization(false);
            var values = this.GetDefaultFormDataFields();
            values.RemoveAll(x => x.Key == "conflict");
            var responseMsgT = /*await*/ this.Account.HttpClient.PostAsync(Urls.DownloadToken, 
                values.ToFormUrlEncodedContent());
            var responseMsg = responseMsgT.Result;

            var responseStrT = /*await*/ responseMsg.Content.ReadAsStringAsync();
            var responseStr = responseStrT.Result;
        ...
        public /*async Task<T>*/ T Publish<T>(
            string sourceFullPath) where T : CloudStructureEntryBase
        {
        ...
        public /*async Task<T>*/ T Unpublish<T>(
            string publicLink) where T : CloudStructureEntryBase
        {
        ...
        public async Task<Data.File> RestoreFileFromHistory(
            string sourceFullPath, long historyRevision, bool rewriteExisting, 
            string newFileName = null)
        {
            ...
            if (this.Account.Has2GBUploadSizeLimit)
            {
                throw new CloudClientException(
                    "The current operation is not supported for your account. " +
                    "Please, upgrade your tariff plan.",
                    ErrorCode.NotSupportedOperation);
            }

            var histories = /*await*/ this.GetFileHistory(sourceFullPath);
            //var histories = historiesT.Result;
            var history = histories.FirstOrDefault(x => x.Revision == historyRevision);
            if (history == null)
            {
                throw new CloudClientException(
                    "History not exists by specified revision number.", nameof(historyRevision), 
                    ErrorCode.HistoryNotExists);
            }
            ...
            var newFullPath = rewriteExisting 
                ? sourceFullPath : 
                this.GetParentCloudPath(sourceFullPath) + newFileName;

            var created = /*await*/ this.CreateFileOrFolder(true, newFullPath, 
                history.Hash, history.SizeBytes, rewriteExisting);
        ...
        public /*async Task<History[]>*/ History[] GetFileHistory(string sourceFullPath)
        {
            Debug.Assert(!string.IsNullOrEmpty(sourceFullPath), "Is null or empty.");

            //await 
            this.Account.CheckAuthorization(false);
            ...
            values.RemoveAll(x => x.Key == "conflict");
            var responseMsgT = /*await*/ this.Account.HttpClient.PostAsync(
                Urls.History, values.ToFormUrlEncodedContent(), sourceFullPath, 
                this.Account.Email, this.Account.AuthToken);

            var responseMsg = responseMsgT.Result;
            if (responseMsg.StatusCode == HttpStatusCode.NotFound)
            {
                throw new CloudClientException(
                    "The file by specified path does not exists.", nameof(sourceFullPath), 
        ...
        public /*async Task*/ bool Remove(string sourceFullPath)
        {
            Debug.Assert(!string.IsNullOrEmpty(sourceFullPath), "Is null or empty.");
            ...
            var values = this.GetDefaultFormDataFields(sourceFullPath);
            //await 
            this.Account.HttpClient.PostAsync(Urls.Remove, values.ToFormUrlEncodedContent());
            return true;
        }
        ...
        public /*async Task<T>*/ T Rename<T>(string sourceFullPath, string name) where 
            T : CloudStructureEntryBase
        {
            Debug.Assert(!string.IsNullOrEmpty(sourceFullPath), "Is null or empty.");
            ...
            var responseMsgT = /*await*/ this.Account.HttpClient.PostAsync(Urls.Rename, 
                values.ToFormUrlEncodedContent());
            var responseMsg = responseMsgT.Result;
        ...
        public /*async Task<T>*/ T Copy<T>(string sourceFullPath, string destFolderPath) 
        ...
        /// <param name="destFolderPath">The destination path of entry.</param>
        /// <returns>The entry info in the new location of cloud.</returns>
        public /*async Task<T>*/ T Move<T>(string sourceFullPath, string destFolderPath) 
            where T : CloudStructureEntryBase
        {
        ...
        public /*async Task<Folder>*/ Folder CreateFolder(string fullFolderPath)
        {
            ...
            //(string NewName, string NewPath) createdFolder = createdFolderT.Result;
        ...
        public /*async Task<FileInfo>*/ FileInfo DownloadItemsAsZIPArchive(
            List<string> filesAndFoldersPaths, string destZipArchiveName, string destFolderPath)
        {
            ...
            destZipArchiveName = string.IsNullOrEmpty(destZipArchiveName) 
            ...
            {
                //await 
                this.DownloadItemsAsZIPArchive(filesAndFoldersPaths, stream);
            }
        ...
        /// <returns>The simple task.</returns>
        public /*async Task*/ bool DownloadItemsAsZIPArchive
        (
            List<string> filesAndFoldersPaths, 
            Stream destStream
        )
        {
            ...
            //await 
            file.NetworkStream.CopyToAsync(file.Length, destStream, 
                this.ProgressChangedEvent, this, this.cancelToken);
        ...
        public /*async Task<*/(Stream NetworkStream, long Length)/*>*/ DownloadItemsAsZIPArchive(List<string> filesAndFoldersPaths)
        {
            ...
            var networkStreamT = /*await*/ this.Account.HttpClient.GetStreamAsync(link);
            Stream networkStream = networkStreamT.Result;
        ...
        public /*async Task<string>*/ string GetDirectLinkZIPArchive(
            List<string> filesAndFoldersPaths, string destZipArchiveName)
        {
            ...
            for (var i = 0; i < filesAndFoldersPaths.Count; i++)
            {
                ...
                filesAndFoldersPaths[i] = string.Format("\"{0}\"", 
                    this.GetPathStartEndSlash(filesAndFoldersPaths[i], setAtEnd: false));
            }

            if (!allHasCommonPath)
            {
                throw new CloudClientException(
                    "Some of files or folders have the different common paths." +
                    " All of items should have the common parent folder.",
            ...
            var responseMsgT = /*await*/ this.Account.HttpClient.PostAsync(Urls.CreateZipArchive, values.ToFormUrlEncodedContent());
            var responseMsg = responseMsgT.Result;
        ...
        public async Task<(Stream NetworkStream, long Length)> DownloadFile(string sourceFilePath)
        {
            ...
            string shardUrl = (/*await*/ this.GetShardsInfo()).Get[0].Url;
                       
            var responseMsgT = /*await*/ this.Account.HttpClient.GetAsync(
                shardUrl + sourceFilePath, HttpCompletionOption.ResponseHeadersRead);
        ...
        public /*async Task<Data.File>*/ Data.File UploadFile
        (
            string destFileName, 
            string sourceFilePath, string destFolderPath
        )
        {
            Debug.Assert(
        ...
        /// <param name="destFolderPath">The destination file folder path in the cloud.</param>
        /// <returns>The created file info.</returns>
        public /*async Task<Data.File>*/ Data.File UploadFile(string destFileName, 
            Stream content, string destFolderPath)
        {
            //await 
            this.Account.CheckAuthorization(false);
            ...
            {
                throw new CloudClientException("Path does not exists.", 
                    nameof(destFolderPath), ErrorCode.PathNotExists);
            }
            ...
            using (var streamContent = new ProgressableStreamContent(content, 
                this.ProgressChangedEvent, this.cancelToken))
            {
                using 
                (
                    var responseT = /*await*/ this.Account.HttpClient.PutAsync(
                ...
                )
                {
                    ...
                    var hashT = /*await*/ response.Content.ReadAsStringAsync();
                    var hash = hashT.Result;

                    var createdFile = /*await*/ this.CreateFileOrFolder(true, 
                        destFolderPath + destFileName, hash, fileSize);
                    //(string NewName, string NewPath) createdFile = createdFileT.Result;
        ...
        public /*async Task<Folder>*/ Folder GetFolder()
        {
        ...
        public /*async Task<*/Folder/*>*/ GetFolder(string fullPath)
        {
            ...
            var responseMsgT = /*await*/ this.Account.HttpClient.GetAsync(
                Urls.ItemsList, this.Account.AuthToken, fullPath);
            ...
            return new Folder(this.Account)
            {
                ...
                PublicLink = string.IsNullOrEmpty(deserialized.Weblink) 
                  ? null 
                  : Urls.PublicLink + deserialized.Weblink,
        ...
        (
            bool addFile, string path, string hash = null, long size = 0, 
            bool rewriteExisting = false
        )
        {
            //await 
            this.Account.CheckAuthorization(false);
            ...
            var responseMsgT = /*await*/ this.Account.HttpClient.PostAsync(
                Urls.CreateFileOrFolder, values.ToFormUrlEncodedContent(), 
                addFile ? "file" : "folder");
            var responseMsg = responseMsgT.Result;
            ...
            try
            {
                newPath = responseStr.Deserialize<string>();
            }
            catch (Exception ex)
            {
                Debug.WriteLine("[ex] responseStr Deserialize exception : " + ex.Message);
            }

            string newName = default;
            try
            {
                newName = newPath.Split(new[] { '/' }).Last();
            }
            catch (Exception ex)
            {
                Debug.WriteLine("[ex] NewPath Split exception : " + ex.Message);
            }
        ...
        private/* async Task<ShardsList>*/ ShardsList GetShardsInfo()
        {
            //await 
            this.Account.CheckAuthorization(false);
            var shardsListT = /*await*/ this.Account.HttpClient.GetStringAsync(
                Urls.Dispatcher, this.Account.AuthToken);
            var shardsList = shardsListT.Result;
        ...
        /// <returns>The entry info in the new location of cloud.</returns>
        private /*async Task<T>*/ T MoveOrCopyInternal<T>
        (string sourceFullPath, string destFolderPath, bool move)
            where T : CloudStructureEntryBase
        {
            Debug.Assert(!string.IsNullOrEmpty(sourceFullPath), "Is null or empty.");
            Debug.Assert(!string.IsNullOrEmpty(destFolderPath), "Is null or empty.");
            ...
            {
                throw new CloudClientException(
                    "Destination folder does not exists in the cloud.", 
                    nameof(destFolderPath), 
                    ErrorCode.PathNotExists);
            }
        ...
        private /*async Task<T>*/ T CheckUnknownItemExisting<T>(
            string sourceFullPath) where T : CloudStructureEntryBase
        {
            ...
            T item = null;
            if ((hasFolder && (item = (T)(object)parentFolderItems.Folders
                .FirstOrDefault(x => x.Name == itemName)) == null)
                || (!hasFolder && (item = (T)(object)parentFolderItems.Files
                .FirstOrDefault(x => x.Name == itemName)) == null))
            {
                throw new CloudClientException(
                    "Source item does not exists in the cloud.", 
                    nameof(sourceFullPath), ErrorCode.PathNotExists);
            }
        ...
        private /*async Task<T>*/ T PublishUnpublishInternal<T>(
            string link, bool publish) where T : CloudStructureEntryBase
        {
            ...
            link = publish 
                ? this.GetPathStartEndSlash(link, setAtEnd: false) 
                : link.Replace(Urls.PublicLink, string.Empty);
            ...
            var responseMsgT = /*await*/ this.Account.HttpClient.PostAsync(
                Urls.FileRequest + operationStr, values.ToFormUrlEncodedContent());
            var responseMsg = responseMsgT.Result;
                ...
                || responseMsg.StatusCode == HttpStatusCode.BadRequest)
            {
                throw new CloudClientException(
                    ( publish 
                      ? "The entry by entered path" 
                      : "The entered public link" ) 
                    + " does not exists",
                    nameof(link),
